...
var CorpseGenerator = (function () {
  ...
  function generateLine(analysis, driftBias) {
    /* driftBias: 0 = connected, 1 = drifting */
    var roll = Math.random();
    var connectionWeight = driftBias ? 0.15 : 0.30;
    var rhymeWeight = driftBias ? 0.15 : 0.25;
    var mutationWeight = 0.20;
    var echoWeight = driftBias ? 0.15 : 0.15;
    /* remainder goes to non sequitur */

    var line;
    if (roll < connectionWeight) {
      line = strategyTemplateFill(analysis);
    } else if (roll < connectionWeight + rhymeWeight) {
      line = strategyRhyme(analysis);
    } else if (roll < connectionWeight + rhymeWeight + mutationWeight) {
      line = strategyMutation(analysis);
    } else if (roll < connectionWeight + rhymeWeight + mutationWeight + echoWeight) {
      line = strategySyntacticEcho(analysis);
    } else {
      line = strategyNonSequitur();
    }
